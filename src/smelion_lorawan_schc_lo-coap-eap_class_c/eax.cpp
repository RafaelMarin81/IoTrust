...
void do_omac(const uint8_t key[16], const uint8_t data[], int length,
   ...
   uint8_t data_padded [MAX_PAYLOAD_LEN];
   const uint8_t* xor_pad;

   aes_encrypt_constrained(L,key);
   ...
   for(j = 0; j != total_len; j += 16)
      ...
      for(k = 0; k != 16; k++) mac[k] ^= data_padded[j+k];
        aes_encrypt_constrained(mac,key);
      }
   }
...
//   aes_context ctx;
  ...
  // aes_set_key(key, 16, &ctx);
   ...
   //aes_encrypt_constrained(state, buffer, &aes_key);
   //aes_encrypt_constrained(state, buffer, &ctx);
   ...
   aes_encrypt_constrained(buffer,key);
