...
#ifdef ENABLE_SLEEP
		  // jsanchez XXX the following lines make SME-Lion Fail.
        // thus we disable them here. SME-Fox is not affected.
        // sleep();
...
// Sends the given payload without acknowledgement.
// Returns 0 (NoError) when transmission is successful or one of the MacTransmitErrorCodes otherwise.
uint8_t Sodaq_RN2483::sendClassC(uint8_t port, const uint8_t* payload, uint8_t size)
{
    debugPrintLn("[sendClassC]");

    return macTransmitClassC(STR_UNCONFIRMED, port, payload, size);
}
...
uint16_t Sodaq_RN2483::receive(uint8_t *fport, uint8_t* buffer, uint16_t size,
...
// FOR CLASS C Devices Only
// jsanchez
// In case of receiving a packet through class C, poll the buffer and return the packet
// Copies the latest received packet (optionally starting from the "payloadStartPosition"
// position of the payload) into the given "buffer", up to "size" number of bytes.
// Returns the number of bytes written or 0 if no packet is received since last transmission.
uint16_t Sodaq_RN2483::pollMacRXClassC(uint8_t *fport, uint8_t* buffer, uint16_t size, uint16_t payloadStartPosition)
{
    debugPrintLn("[pollMacRX]");

	debugPrintLn("."); debugPrint("("); debugPrint(this->inputBuffer); debugPrintLn(")");
	// There are chars in the UART buffer
	...
	if (readLn() > 0) {
		...
		if (strstr(this->inputBuffer, " ") != NULL) // to avoid double delimiter search
		{
			...
			onMacRX();
			return receive(fport, buffer, size);
		}
	...
	return receive(fport, buffer, size, payloadStartPosition);
...
// Gets the downlink frame counter
uint32_t Sodaq_RN2483::get_dnctr(void)
{
    debugPrintLn("[getHWEUI]");

    this->loraStream->print("mac get dnctr");
    this->loraStream->print(CRLF);
    ...
    while (millis() - start < DEFAULT_TIMEOUT) {
        ...
        if (readLn() > 0) {
			  return strtoul(inputBuffer, NULL, 10);
        }
    }
...
// Gets the downlink frame counter
uint32_t Sodaq_RN2483::get_upctr(void)
{
    debugPrintLn("[getHWEUI]");

    this->loraStream->print("mac get upctr");
    this->loraStream->print(CRLF);
...
// Sets the downlink frame counter.
// Returns true on success.
bool Sodaq_RN2483::set_dnctr(uint32_t dnctr)
{

	 char buf[10] = {0};

	 snprintf(buf, sizeof(buf), "%u", (unsigned int) dnctr);

	 return setMacParam("dnctr", buf);

}

// Sets the uplink frame counter.
// Returns true on success.
bool Sodaq_RN2483::set_upctr(uint32_t upctr)
{

	 char buf[10] = {0};

	 snprintf(buf, sizeof(buf), "%u", (unsigned int) upctr);

	 return setMacParam("upctr", buf);
}
...
bool Sodaq_RN2483::resetDevice()
{
    ...
    if (expectString(STR_DEVICE_TYPE_RN, 1000)) {
...
bool Sodaq_RN2483::joinNetwork(const char* type)
{
    debugPrintLn("[joinNetwork]");
	 /*
	  * If you're far away from the GW, employ a lower value e.g. dr 0 or 1.
	  */
	 sendCommand("mac set dr 4");

	 /*
	  * Use only with RN2483 updated to firmware v1.0.5+. See the Microchip RN2483 documentation.
	  * Activate LoRaWAN Class C mode.
	  */
	 // sendCommand("mac set class c");

	 /*
	  * Further improve the coverage area of the JOIN command by setting the code rate to a
	  * more robust configuration, i.e. cr 4/8
	  *
	  * NOTE that after the JOIN OTAA is completed, the code rate is set back to 4/5.
	  */
	 ...
	 /*
	  * Configure the maximum output power for transmission.
	  * This further improves the coverage of the device. Use if you're away from the GW.
	  */
...
uint8_t Sodaq_RN2483::macTransmit(const char* type, uint8_t port, const uint8_t* payload, uint8_t size)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        debugPrint(static_cast<char>(NIBBLE_TO_HEX_CHAR(HIGH_NIBBLE(payload[i]))));
        debugPrint(static_cast<char>(NIBBLE_TO_HEX_CHAR(LOW_NIBBLE(payload[i]))));
    }
...
// For CLASS C DEVICES ONLY
uint8_t Sodaq_RN2483::macTransmitClassC(const char* type, uint8_t port, const uint8_t* payload, uint8_t size)
{
    debugPrintLn("[macTransmitClassC]");
    debugPrint(STR_CMD_MAC_TX);
    debugPrint(type);
    debugPrint(port);
    debugPrint(" ");
