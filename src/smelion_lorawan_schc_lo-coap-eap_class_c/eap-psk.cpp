...
//	aes_context ctx;
	...
	for (it=0; it<16; it=it+2){
		// aux = random_rand();  // CONTIKI ARCH
		aux = random(0, 65535U);  // Arduino
...
//	aesencrypt(ct, output, &ctx);
	aes_encrypt_constrained(output,PSK);
...
//	aesencrypt(ct, ak, &ctx);
	aes_encrypt_constrained(ak,PSK);
	//printf_hex(ak,16);
...
//	aesencrypt(ct, kdk, &ctx);
	aes_encrypt_constrained(kdk,PSK);
...
//	aesencrypt(rand_p, output, &ctx);
	aes_encrypt_constrained(output,kdk);	
	...
	aes_encrypt_constrained(tek_key,kdk);
...
//	aesencrypt(ct, msk_key, &ctx);
	aes_encrypt_constrained(msk_key,kdk);
