...
#define DEFAULT_INPUT_BUFFER_SIZE 600
#define DEFAULT_RECEIVED_PAYLOAD_BUFFER_SIZE 500
#define DEFAULT_TIMEOUT 120
#define RECEIVE_TIMEOUT 9000
...
public:
    ...
    // FOR CLASS C Devices Only
	 // jsanchez
	 // Use this call exclusively for Class C devices
    // Sends the given payload without acknowledgement.
    // Returns 0 (NoError) when transmission is successful or one of the MacTransmitErrorCodes otherwise.
    uint8_t sendClassC(uint8_t port, const uint8_t* payload, uint8_t size);
    ...
    uint16_t receive(uint8_t *fport, uint8_t* buffer, uint16_t size, uint16_t payloadStartPosition = 0);
    ...
    // position of the payload) into the given "buffer", up to "size" number of bytes.
    // Returns the number of bytes written or 0 if no packet is received since last transmission.
    uint16_t pollMacRXClassC(uint8_t *fport, uint8_t* buffer, uint16_t size, uint16_t payloadStartPosition = 0);
    ...
    uint32_t get_dnctr(void);

	 // Sets the downlink frame counter
    // Returns true on success.
	 bool set_dnctr(uint32_t dnctr);

    // Gets the uplink frame counter
    uint32_t get_upctr(void);

	 // Sets the uplink frame counter
    // Returns true on success.
	 bool set_upctr(uint32_t upctr);
...
private:
    ...
    bool isRN2903;
	 ...
	 // jsanchez {
	 // The LoRaWAN fPort of the last downlink received packet.
	 uint8_t rx_fport;
    ...
    // Sends a a payload and blocks until there is a response back, or the receive windows have closed,
    // or the hard timeout has passed.
    uint8_t macTransmitClassC(const char* type, uint8_t port, const uint8_t* payload, uint8_t size);
